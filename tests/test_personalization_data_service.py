...
def make_diagnostic(root: Path) -> SimpleNamespace:
    return SimpleNamespace(
        ...
        state=SimpleNamespace(ready_for_basic_mode=True, cuda_runtime_not_verified=False),
        gpu_devices=(),
        warnings=(),
        to_json=lambda: "{}",
    )
...
def build_workspace(settings, paths, catalog, clip_service, personalization_service):
    return WorkspaceViewModel(
        ...
        media_service=SimpleNamespace(
            verify_media_tools=lambda: SimpleNamespace(ffmpeg=SimpleNamespace(available=True, version="6.1"), ffprobe=SimpleNamespace(available=True, version="6.1"), warnings=(), available=True),
        ...
        transcription_service=SimpleNamespace(
            ...
            list_models=lambda: (
                SimpleNamespace(
                    ...
                    path=str(paths.models_directory / "transcription" / "small"),
                    notes="Modelo instalado",
                ),
            ),
            get_model_status=lambda model_name: SimpleNamespace(
                model_name=model_name,
                status=SimpleNamespace(value="installed" if model_name == "small" else "not_installed"),
                path=str(paths.models_directory / "transcription" / model_name),
                notes="Modelo instalado" if model_name == "small" else "Modelo no instalado",
            ),
            verify_model=lambda model_name: SimpleNamespace(
                model_name=model_name,
                status=SimpleNamespace(value="installed" if model_name == "small" else "not_installed"),
                path=str(paths.models_directory / "transcription" / model_name),
                notes="Modelo instalado" if model_name == "small" else "Modelo no instalado",
