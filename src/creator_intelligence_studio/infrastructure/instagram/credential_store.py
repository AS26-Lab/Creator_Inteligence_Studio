"""Almacenamiento local de credenciales para Instagram."""
...
class InstagramCredentialBundle:
...
class InstagramCredentialStore(Protocol):
    def save(self, reference: str, bundle: InstagramCredentialBundle) -> None: ...
    def load(self, reference: str) -> InstagramCredentialBundle | None: ...
...
class DevelopmentInstagramCredentialStore:
    ...
    def load(self, reference: str) -> InstagramCredentialBundle | None:
        ...
        return InstagramCredentialBundle(
            ...
            provider=str(payload.get("provider") or "instagram_login"),
        )
...
class EncryptedLocalInstagramCredentialStore:
    def __init__(self, root: Path, *, secret: str | None = None) -> None:
        self._root = root
        self._root.mkdir(parents=True, exist_ok=True)
        self._secret = secret or os.environ.get("CIS_INSTAGRAM_CREDENTIAL_SECRET")
        if not self._secret:
            raise ValueError("Se requiere CIS_INSTAGRAM_CREDENTIAL_SECRET para el almacén cifrado de Instagram.")
